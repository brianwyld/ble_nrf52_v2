...
bool CFMgr_setElement(uint16_t key, void* data, uint8_t len) {
    ...
    if (idx<0) {
        ...
        uint8_t klen = getIdxLen(idx);
        if (len==klen) {
            // optimisation : only write new value iff different from current nvm
            if (isNVMDifferent((uint8_t*)data, getIdxOff(idx), len)) {
                // Write data
                cfgLockW();
                ret = hal_bsp_nvmWrite(getIdxOff(idx), len, (uint8_t*)data);
                cfgUnlockW();
            }
        } else {
...
static bool isNVMDifferent(uint8_t* newdata, uint16_t nvmoff, uint8_t len) {
    for(int i=0;i<len;i++) {
        if (newdata[i] != hal_bsp_nvmRead8(nvmoff+i)) {
